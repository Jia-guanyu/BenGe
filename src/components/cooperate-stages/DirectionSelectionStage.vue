...
const submitSelectedDirections = async () => {
  ...
  /* try {
    ...
    触发 submission 事件，将数据传递给父组件
    emit('submission', {
      message: `用户已提交 ${selectedDirections.value.length} 个剧本方向`,
      details: selectedDirections.value.map(dir => dir.content).join(', '),
...
.confirmSubmit-btn:hover:not(:disabled) {
  background-color: #0D9F6E;
  transform: translateY(-2px);
  box-shadow: 0 4px 8px rgba(16, 185, 129, 0.3);
}
.confirmSubmit-btn:disabled {
  background-color: #ccc;
  cursor: not-allowed;
}
